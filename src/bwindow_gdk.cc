...
#endif
	{
		...
		if(!no_keys)
		{
			...
			for(unsigned i = 0; s[i] != 0; i++)
			{
				...
				MOD{
					B->modify_bg(Gtk::STATE_NORMAL,MAIN_COLOR_GDK);
...
void SurfBWindow::refresh_image(const std::string& script, const std::string& image, const int aa, bool full, bool max_res, bool max_res2)
{
//std::cout<<"action at "<<m_last_action<<std::endl;
...

	{
		...
		{
			std::cerr << "could not write to file "<< script << std::endl;
			exit(1437);
		}
		...
		for(unsigned kk = 0; kk < data.size(); kk++)
		{
			write_surf(data[kk],kk,f);
		}
...
void SurfBWindow::refresh_display(const std::string& image, bool )
{
	...
	//some_gc.create(get_window());
		...
		{
			try{
				Glib::RefPtr<Gdk::Pixbuf> surface2 = Gdk::Pixbuf::create_from_file(image.c_str());
				Glib::RefPtr<Gdk::Pixbuf> sface2 = surface2->scale_simple(width-40,height-40,Gdk::INTERP_BILINEAR);
				dr->draw_pixbuf(some_gc,sface2,0,0,20,20,width-40,height-40,Gdk::RGB_DITHER_NONE,0,0);
				}
...
//Bild berechnen
	...
	refresh_image(TEMP_ROOT_SEP +"surfb_f_p.pic",TEMP_ROOT_SEP +"surfb_f_p.ppm",global_data.antialiasing,true,true,false);
...
void SurfBWindow::refresh_print(const std::string& image)
{
	...
	if(finished)
	{
		...
		Glib::RefPtr<Gdk::Pixbuf> surface2 = Gdk::Pixbuf::create_from_file((image).c_str());
		//Glib::RefPtr<Gdk::Pixbuf> sface2 = surface2->scale_simple(width-40,height-40,Gdk::INTERP_BILINEAR);
		//bs->draw_pixbuf(some_gc,sface2,0,0,0,0,width,height,Gdk::RGB_DITHER_NONE,0,0);
		surface2->copy_area(0,0,opt.print_resolution,opt.print_resolution,bs,0,0);
	  ...
	  dialog.add_filter(filter_ppm);
		...
		if(result == Gtk::RESPONSE_OK)
		{
			...
			if(do_png)
			{
				...
				refresh_image(TEMP_ROOT_SEP+"surfb_s.pic",TEMP_ROOT_SEP+"surfb_s.ppm",global_data.antialiasing,true,true);
...
//FIXME: Gtk::Stock::DELETE does not work under windows (with Gtkmm 2.10)
	...
	Glib::RefPtr<Gdk::Pixbuf> flag_galicia = Gdk::Pixbuf::create_from_xpm_data(flag_ga_ES);
	icon_size = flag_galicia->get_height();
	git->add_builtin_icon("flag_ga",icon_size,flag_galicia);
	...
	mr_AG->add(Gtk::Action::create_with_icon_name("LangGA","flag_ga","Galego","Galego"), sigc::bind(sigc::mem_fun(*this, &SurfBWindow::restart_with_lang),"ga_ES.UTF-8"));
...
void do_postpare()
{
	std::remove((TEMP_ROOT_SEP+"surfb.ppm").c_str());
	std::remove((TEMP_ROOT_SEP+"surfb.pic").c_str());
	std::remove((TEMP_ROOT_SEP+"surfb_f.ppm").c_str());
	std::remove((TEMP_ROOT_SEP+"surfb_f.pic").c_str());
}
